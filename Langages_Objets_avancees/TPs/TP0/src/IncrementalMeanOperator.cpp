 ...
 */
template <class T>
IncrementalMeanOperator<T>::IncrementalMeanOperator() noexcept(false) : count(0)
{
 ...
 */
template <class T>
T IncrementalMeanOperator<T>::operator()(const T & currentMean,
										 const T & newValue)
{
	// E(X_n+1) = (n*E(X_n) + x_n+1) / (n+1)
	T newMean = (static_cast<T>(count) * currentMean + newValue) /
...
template class IncrementalMeanOperator<double>;
